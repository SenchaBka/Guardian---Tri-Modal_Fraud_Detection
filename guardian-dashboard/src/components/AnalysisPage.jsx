...
import ResultPanel from "./Resultpanel.jsx/index.js";
import HistoryTable from "./Historytable.jsx/index.js";
import {
  simulateNumerical,
  simulateNLP,
  simulateVoice,
...
function HighlightedText({ text }) {
  ...
  for (const w of RISK_WORDS) {
    if (text.toLowerCase().includes(w)) riskMatches.push(w);
  }
  if (!typoMatches.length && !riskMatches.length)
    return <span className="font-mono text-xs text-ink">{text}</span>;
  const regex = new RegExp(
    `(${[...typoMatches, ...riskMatches].map((s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|")})`,
    "gi",
  );
  return (
    <span className="font-mono text-xs text-ink">
      {text.split(regex).map((p, i) => {
        if (typoMatches.some((m) => m.toLowerCase() === p.toLowerCase()))
          return (
            <span
              key={i}
              className="bg-danger/20 text-danger border-b border-danger font-bold"
            >
              {p}
            </span>
          );
        if (riskMatches.some((m) => m.toLowerCase() === p.toLowerCase()))
          return (
            <span
              key={i}
              className="bg-amber/20 text-amber border-b border-amber"
            >
              {p}
            </span>
          );
...
function StreamRow({
  ...
  return (
    <div
      className={`rounded-lg border transition-colors ${enabled ? "border-teal/20 bg-bg-3" : "border-border bg-bg-3"}`}
    >
      {/* header */}
        ...
        {required ? (
          ...
          <label className="relative w-8 h-[18px] cursor-pointer flex-shrink-0">
            ...
            <div
              ...
              ${enabled ? "bg-teal border-teal" : "bg-bg-4 border-border"}`}
            />
            <div
              className={`absolute top-0.5 w-3 h-3 rounded-full transition-all
              ${enabled ? "left-[17px] bg-bg-0" : "left-0.5 bg-ink-muted"}`}
            />
          </label>
        )}
      </div>
...
// ── main page ─────────────────────────────────────────────────────────────────
export default function AnalysisPage({ stats, onNewResult }) {
  // form state
  ...
  // stream toggles (driven by channel)
  ...
  const handleAnalyze = async () => {
    ...
    const nlp =
      nlpEnabled && merchant ? simulateNLP(merchant, narrative) : null;
    const voice =
      voiceEnabled && voiceSample
        ? simulateVoice(voiceSample.name, voiceSample.isDeepfake)
        : null;
          ...
          <div className="card-body space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted text-xs font-mono">
                    $
                  </span>
                  <input
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <StreamRow
                  name="Numerical"
                  owner="Ivan · XGBoost + RF · required"
                  required
                  enabled
                  icon={
                    <svg
                ...
                <StreamRow
                  name="NLP"
                  owner="Luis · FinBERT + spaCy NER"
                  enabled={nlpEnabled}
                  onToggle={() => setNlpEnabled((v) => !v)}
                  icon={
                    <svg
                      width="13"
                      height="13"
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {[
                      ...
                      ["Crypto scam", "BitCoinExpress", "buy bitcoin winner"],
                    ].map(([l, m, n]) => (
                      <button
                        key={l}
                        onClick={() => {
                ...
                <StreamRow
                  name="Voice"
                  owner="Arsenii · WavLM deepfake detection"
                  ...
                  icon={
                    ...
                    >
                      <path
                        d="M8 2C6 2 4 3.5 4 5.5C4 7.5 5 9 8 11C11 9 12 7.5 12 5.5C12 3.5 10 2 8 2Z"
                        stroke="currentColor"
                        strokeWidth="1.3"
                        fill="none"
                      />
                ...
                >
                  ...
                  {voiceSample && (
                    <p className="text-[9px] font-mono text-ink-dim">
                      ...
                      <span
                        className={
                          voiceSample.isDeepfake ? "text-danger" : "text-teal"
                        }
                      >
                        {voiceSample.name}
                      </span>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-md
                  ...
                  ${
                    canAnalyze && !loading
                      ? "bg-teal text-bg-0 hover:brightness-110 cursor-pointer"
                      : "bg-bg-3 text-ink-dim cursor-not-allowed opacity-50"
                  }`}
              >
                {loading ? (
                  <>
                    <div className="w-3.5 h-3.5 border-2 border-bg-0/30 border-t-bg-0 rounded-full animate-spin-slow" />
                    Analyzing…
                  </>
                ) : (
                  <>
